...
if($system eq 'E'){
	...
	print "Invalid system in test code: not encoder or decoder. Exiting script\n";
	exit;
}
if($testtype eq 'R'){
	print "Reset test\n";
...
# Create output log file for test (./aprsc_test_out/testname.out)
my $destfilepath = "./aprsc_test_out/";
mkdir $destfilepath unless -d $destfilepath;
my $testname = substr($test,0,7);
my $destfilename = $testname . ".out";
...
# call sim script to run normal test bench for APRSC. Eventually want RTL and Netlist
...
my $simoutput = system("./sim.csh -n -ng -run >$netlistlogname");
#print LOGFILE $simoutput;
...
my $simoutput2 = system("./sim.csh -r -ng -run >$rtllogname");
...
print "\n-----------------------------------------------------------------------------\n";
print "\nWriting results to APRSCTests database\n";
if($netlisterror == 1 || $rtlerror == 1){
	$qh = $dbh->prepare("INSERT INTO APRSCTests VALUES ('$testname', '$currenttester', '$start', '$end', 'failed', '$assignednum', '$currentgithash', '$currentsyntime');");
	$qh->execute;
}
else{ # passed
	$qh = $dbh->prepare("INSERT INTO APRSCTests VALUES ('$testname', '$currenttester', '$start', '$end', 'passed', '$assignednum', '$currentgithash', '$currentsyntime');");
	$qh->execute;
}
...
my $hash;
my $syntime;
$qh = $dbh->prepare("SELECT * FROM APRSCTests");
$qh->execute;
...
my $subject = "Automated APRSC Test Finished: Test " . $testname;
print "\n-----------------------------------------------------------------------------\n";
