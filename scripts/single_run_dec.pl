...
my $test;
if ($t eq "") {
	print "Enter the test code (ex: DRNAA16): ";
	$test = <>;
} else {
	$test = $t;
}
...
if($system eq 'D'){
	...
	print "Invalid system in test code: not decoder. Exiting script\n";
	exit;
}
if($testtype eq 'R'){
	print "Reset test\n";
...
# Create output log file for test (./dec_test_out/testname.out)
my $destfilepath = "./dec_test_out/";
mkdir $destfilepath unless -d $destfilepath;
my $testname = substr($test,0,7);
my $destfilename;
if($netlist){
	$destfilename = "dec" . $testname . "NET.out";
}
else{
	$destfilename = "dec" . $testname . "RTL.out";
}
...
my $currentsyntime;
if($netlist){
	$currentsyntime = `date -r ../dec/dc_shell_cmp_scan.log +"%D_%R:%S"`;
}
else{
	$currentsyntime = "";
}
...
my $rtllogname;
if($netlist){
	# call sim script to run normal test bench for DEC. Eventually want RTL and Netlist
...
if($netlist){
	print "\n-----------------------------------------------------------------------------\n";
	print "\nWriting results to DECNETTests database\n";
	if($netlisterror == 1){
		$qh = $dbh->prepare("INSERT INTO DECNETTests VALUES ('$testname', '$currenttester', '$start', '$end', 'failed', '$assignednum', '$currentgithash', '$currentsyntime');");
		$qh->execute;
	}
	else{ # passed
		$qh = $dbh->prepare("INSERT INTO DECNETTests VALUES ('$testname', '$currenttester', '$start', '$end', 'passed', '$assignednum', '$currentgithash', '$currentsyntime');");
		$qh->execute;
	}
}
else{
	print "\n-----------------------------------------------------------------------------\n";
	print "\nWriting results to DECRTLTests database\n";
	if($rtlerror == 1){
		$qh = $dbh->prepare("INSERT INTO DECRTLTests VALUES ('$testname', '$currenttester', '$start', '$end', 'failed', '$assignednum', '$currentgithash');");#, '$currentsyntime');");
		$qh->execute;
	}
	else{ # passed
		$qh = $dbh->prepare("INSERT INTO DECRTLTests VALUES ('$testname', '$currenttester', '$start', '$end', 'passed', '$assignednum', '$currentgithash');");#, '$currentsyntime');");
		$qh->execute;
	}
}
...
if($netlist){
	$qh = $dbh->prepare("SELECT * FROM DECNETTests");
	$qh->execute;
	...
	$qh = $dbh->prepare("SELECT * FROM DECRTLTests");
	$qh->execute;
...
my $subject = "Automated DEC Test Finished: Test " . $testname;
print "\n-----------------------------------------------------------------------------\n";
