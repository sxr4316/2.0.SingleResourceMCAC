...
// APRSC Communication Functions
	...
	/*---------------------------------------------------------------------------------------
		Description - Write the relevant data to the APRSC & request it to start processing

		Usage       - aprsc_send(tr, rate, dq);	
	---------------------------------------------------------------------------------------*/
	...
	/*---------------------------------------------------------------------------------------
		Description - Read from APRSC. If ack = 0 --> return 0. If ack = 1 --> Read the
				  rest of the data. enc_dec = 0 for encoder (only reads a2p & se) and
				  enc_dec = 1 for decoder (reads a2p, se, and sr).

		Usage       - aprsc_read(0, &a2p, &se, &sr); for encoder
				  aprsc_read(1, &a2p, &se, &sr); for decoder
	---------------------------------------------------------------------------------------*/
